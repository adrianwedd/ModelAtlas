...
from atlas_cli.search import cli, display_results, load_models, search_models
...
def mock_catalog_path(tmp_path):
    ...
    dummy_data = [
        ...
        {
            "name": "Gemma",
            "summary": "A lightweight, state-of-the-art open model from Google",
        },
...
def test_load_models_file_not_found(tmp_path):
    ...
    with patch("atlas_cli.search.console.print") as mock_print:
        ...
        mock_print.assert_called_with(
            f"[bold red]Catalog not found at {non_existent_path}.[/]"
        )
...
def test_cli_with_results(
    ...
    mock_search_models.assert_called_once_with(
        "test_query", [{"name": "Model1", "summary": "Summary1"}], top_k=5
    )
    mock_display_results.assert_called_once_with(
        [{"name": "Model1", "summary": "Summary1"}]
    )
...
def test_cli_no_results(
    mock_console_print, mock_search_models, mock_load_models, mock_catalog_path
):
    mock_load_models.return_value = [{"name": "Model1", "summary": "Summary1"}]
...
def test_load_models_returns_empty_on_corrupted_catalog(tmp_path):
    ...
    assert result == [], "Corrupted catalog must return empty list, not raise"
