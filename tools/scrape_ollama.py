...
async def fetch_manifest(
    client: httpx.AsyncClient, model: str, tag: str = "latest"
) -> dict | None:
    url = f"https://registry.ollama.ai/v2/library/{model}/manifests/{tag}"
    return await fetch_json(client, url)


async def decode_manifest_config(
    client: httpx.AsyncClient, model_name: str, digest: str
) -> dict | None:
...
async def scrape_tags_page(client: httpx.AsyncClient, model_name: str) -> list[dict]:
    ...
    for item in soup.select("div.group.px-4.py-3"):
        ...
        details_div = item.find(
            "div", class_="flex text-neutral-500 text-xs items-center"
        )
        digest = ""
        last_updated = ""
        if details_div:
            ...
            last_updated_match = re.search(
                r"·\s*(.*)", details_div.get_text(strip=True)
            )
        ...
        if manifest:
            ...
            if manifest.get("config") and manifest["config"].get("digest"):
                ...
                cfg = await decode_manifest_config(
                    client, model_name, tag_entry["digest"]
                )
                if cfg:
                    tag_entry["config"] = cfg
                    tag_entry["context_length"] = cfg.get("parameters", {}).get(
                        "context_window"
                    )
            ...
            for layer in manifest.get("layers", []):
                ...
                if layer.get("annotations"):
                    if "ollama.context_window" in layer[
                        "annotations"
                    ] and not tag_entry.get("context_length"):
                        tag_entry["context_length"] = layer["annotations"][
                            "ollama.context_window"
                        ]
                    if "ollama.input_type" in layer[
                        "annotations"
                    ] and not tag_entry.get("model_type"):
                        tag_entry["model_type"] = layer["annotations"][
                            "ollama.input_type"
                        ]
...
async def process_model(
    ...
    async with semaphore:
        ...
        if existing.get("page_hash") == detail.get("page_hash"):
            logger.info(
                "Page hash for %s matches existing data. Skipping detailed scrape.",
                name,
            )
            data = existing
        else:
            logger.info(
                "Page hash for %s changed or no existing data. Performing full scrape.",
                name,
            )
...
async def scrape_ollama_models(
    concurrency: int = 5, debug_model: str | None = None, use_cache: bool = True
) -> list[dict]:
    ...
    async with httpx.AsyncClient() as client:
        ...
        if debug_model:
            ...
            if not names:
                logger.error(
                    "Debug model '%s' not found in Ollama.com initial list.",
                    debug_model,
                )
                return []
    ...
    logger.info(
        "Ollama.com scraping complete — %s models stored in %s",
        len(results),
        OLLAMA_MODELS_DIR,
    )
    return results
